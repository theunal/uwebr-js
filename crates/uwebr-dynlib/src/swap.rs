...
mod tests {
    ...
    #[test]
    fn dynlib_versioned_filename_long_name() {
        let long_name = "VeryLongComponentNameThatExceedsNormalLimits";
        let name = versioned_filename(long_name, 1);
        assert!(name.contains(long_name));
        assert!(name.contains("v1"));
        assert!(name.ends_with(library_extension()));
    }

    #[test]
    fn dynlib_hot_swap_manager_non_existent_dir() {
        let mgr = HotSwapManager::new(
    ...
    #[test]
    fn dynlib_swap_error_load_failed_display_contains_path() {
        let err = SwapError::LoadFailed {
            path: PathBuf::from("/some/path/lib.dll"),
            error: "file not found".into(),
        };
        let msg = format!("{err}");
        assert!(msg.contains("/some/path/lib.dll"));
        assert!(msg.contains("file not found"));
    }

    #[test]
    fn dynlib_swap_error_render_display_contains_detail() {
        let err = SwapError::RenderFailed {
            detail: "function returned null pointer".into(),
        };
        let msg = format!("{err}");
        assert!(msg.contains("render failed"));
        assert!(msg.contains("function returned null pointer"));
    }

    #[test]
    fn dynlib_swap_error_css_display_contains_error() {
        let err = SwapError::CssParseFailed {
            error: "unexpected token at line 5".into(),
        };
        let msg = format!("{err}");
        assert!(msg.contains("css parse failed"));
        assert!(msg.contains("unexpected token at line 5"));
    }

    #[test]
    fn dynlib_swap_error_is_std_error() {
        let err: Box<dyn std::error::Error> = Box::new(SwapError::RenderFailed {
            detail: "test".into(),
        });
        assert!(format!("{err}").contains("render failed"));
    }
    ...
    #[test]
    fn dynlib_hot_swap_manager_library_dir() {
        let dir = PathBuf::from("/some/library/dir");
        let mgr = HotSwapManager::new(dir.clone(), "Widget".into());
        assert_eq!(mgr.library_dir(), dir.as_path());
    }

    #[test]
    fn dynlib_hot_swap_manager_component_name() {
        let mgr = HotSwapManager::new(PathBuf::from("/tmp"), "MyCounter".into());
        assert_eq!(mgr.component_name(), "MyCounter");
    }

    #[test]
    fn dynlib_version_starts_at_zero() {
        let mgr = HotSwapManager::new(PathBuf::from("/tmp"), "App".into());
        assert_eq!(mgr.version(), 0);
    }

    #[test]
    fn dynlib_versioned_filename_version_zero() {
        let name = versioned_filename("App", 0);
    ...
    #[test]
    fn dynlib_next_version_path_preserves_dir() {
        let dir = PathBuf::from("/custom/dir");
        let mut v = 0;
        let p = next_version_path(&dir, "App", &mut v);
        assert!(p.starts_with("/custom/dir"));
        assert_eq!(v, 1);
    }

    #[test]
    fn dynlib_versioned_filename_with_special_chars() {
        let name = versioned_filename("my-cmp", 5);
        assert!(name.contains("my-cmp"));
        assert!(name.contains("v5"));
    }

    #[test]
    fn dynlib_hot_swap_manager_initial_version_zero_after_new() {
        let mgr = HotSwapManager::new(PathBuf::from("/tmp"), "Test".into());
        assert_eq!(mgr.version(), 0);
        assert!(mgr.render().is_none());
        assert!(mgr.css().is_none());
    }

    #[test]
    fn dynlib_next_version_path_sequential_increments() {
        let dir = PathBuf::from("/tmp");
        let mut v = 100;
        let p1 = next_version_path(&dir, "App", &mut v);
        assert!(p1.to_string_lossy().contains("v101"));
        let p2 = next_version_path(&dir, "App", &mut v);
        assert!(p2.to_string_lossy().contains("v102"));
        assert_eq!(v, 102);
    }
}
