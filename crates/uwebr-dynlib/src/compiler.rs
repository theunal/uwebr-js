...
<style>.app { color: red; }</style>"#;
  ...
  <p>Hello</p>
    ...
    #[test]
    fn dynlib_extract_css_multiple_rules() {
        let content = r#"<div>Hi</div>
<style>
body { margin: 0; }
.app { color: red; font-size: 14px; }
.container { display: flex; }
</style>"#;
        let css = extract_css(content).unwrap();
        assert!(css.contains("body { margin: 0; }"));
        assert!(css.contains(".app { color: red; font-size: 14px; }"));
        assert!(css.contains(".container { display: flex; }"));
    }

    #[test]
    fn dynlib_extract_css_media_queries() {
        let content = r#"<div>Hi</div>
<style>
@media (max-width: 768px) {
  ...
  .sidebar { display: none; }
        ...
        assert!(css.contains(".sidebar { display: none; }"));
    }

    #[test]
    fn dynlib_extract_css_important() {
        let content = r#"<div>Hi</div>
<style>
.app { color: red !important; z-index: 9999 !important; }
</style>"#;
        let css = extract_css(content).unwrap();
        assert!(css.contains("color: red !important;"));
        assert!(css.contains("z-index: 9999 !important;"));
    }

    #[test]
    fn dynlib_extract_css_with_strings() {
        let content = r#"<div>Hi</div>
<style>
.app::before { content: "style"; }
</style>"#;
        let css = extract_css(content).unwrap();
        assert!(css.contains(r#"content: "style""#));
    }

    #[test]
    fn dynlib_extract_css_empty_style_tags() {
        let content = "<div>Hi</div><style></style>";
        assert_eq!(extract_css(content), None);

        let content2 = "<div>Hi</div><style>   </style>";
        assert_eq!(extract_css(content2), None);
    }

    #[test]
    fn dynlib_extract_html_deeply_nested() {
        let content = r#"<div><section><article><p><span><strong>deep</strong></span></p></article></section></div>"#;
        let html = extract_html(content);
        assert!(html.contains("<strong>deep</strong>"));
        assert!(!html.contains("<style>"));
        assert!(!html.contains("<script>"));
    }

    #[test]
    fn dynlib_extract_html_preserves_component_tags() {
        let content = r#"<div>
<MyWidget prop="val" />
<span>text</span>
</div>"#;
        let html = extract_html(content);
        assert!(html.contains("<MyWidget prop=\"val\" />"));
        assert!(html.contains("<span>text</span>"));
    }

    #[test]
    fn dynlib_extract_tag_div() {
        let content = "before <div>content here</div> after";
        assert_eq!(extract_tag(content, "div"), "content here");
    }

    #[test]
    fn dynlib_extract_tag_span() {
        let content = "before <span>inline text</span> after";
        assert_eq!(extract_tag(content, "span"), "inline text");
    }

    #[test]
    fn dynlib_extract_tag_style_returns_content() {
        let content = "<div>hi</div><style>.a { color: blue; }</style>";
        assert_eq!(extract_tag(content, "style"), ".a { color: blue; }");
    }

    #[test]
    fn dynlib_extract_event_handlers_multiple() {
        let html = r#"<button on:click={save}>Save</button>
<div on:click={cancel}>Cancel</div>
<a on:click={navigate}>Link</a>"#;
        let handlers = extract_event_handlers(html);
        assert_eq!(handlers, vec!["save", "cancel", "navigate"]);
    }

    #[test]
    fn dynlib_extract_event_handlers_complex_expressions() {
        let html = r#"<button on:click={handleClick}>Click</button>"#;
        let handlers = extract_event_handlers(html);
        assert_eq!(handlers, vec!["handleClick"]);
    }

    #[test]
    fn dynlib_to_snake_hello_world() {
        assert_eq!(to_snake("HelloWorld"), "hello_world");
    }

    #[test]
    fn dynlib_to_snake_abc() {
        assert_eq!(to_snake("ABC"), "a_b_c");
    }

    #[test]
    fn dynlib_to_snake_single_char() {
        assert_eq!(to_snake("A"), "a");
        assert_eq!(to_snake("x"), "x");
    }

    #[test]
    fn dynlib_to_snake_no_uppercase() {
        assert_eq!(to_snake("already"), "already");
        assert_eq!(to_snake("my_app"), "my_app");
    }

    #[test]
    fn dynlib_to_snake_consecutive_uppercase() {
        assert_eq!(to_snake("XMLParser"), "x_m_l_parser");
        assert_eq!(to_snake("GETRequest"), "g_e_t_request");
    }

    #[test]
    fn dynlib_library_path_construction() {
        let target_dir = PathBuf::from("/project/target");
        let path = abi::library_path(&target_dir, "App");
        let ext = abi::library_extension();
        assert_eq!(
            path.file_name().unwrap().to_str().unwrap(),
            format!("uwebr_dynlib_App.{ext}")
        );
        assert_eq!(path.parent().unwrap(), Path::new("/project/target"));
    }

    #[test]
    fn dynlib_library_path_nested_dir() {
        let target_dir = PathBuf::from("/a/b/c/target");
        let path = abi::library_path(&target_dir, "MyWidget");
        assert!(path.starts_with("/a/b/c/target"));
        let ext = abi::library_extension();
        assert!(path
            .to_string_lossy()
            .ends_with(&format!("uwebr_dynlib_MyWidget.{ext}")));
    }

    #[test]
    fn dynlib_generate_lib_rs_contains_render_and_css() {
        let project_root = PathBuf::from("/fake/crates/uwebr-dynlib");
        let content = r#"<div class="app">Hello</div>
<style>.app { color: red; }</style>"#;
        let lib_rs = generate_lib_rs(content, "App", &project_root).unwrap();
        assert!(lib_rs.contains("pub extern \"C\" fn render()"));
        assert!(lib_rs.contains("pub extern \"C\" fn css()"));
        assert!(lib_rs.contains("pub extern \"C\" fn cleanup()"));
    }

    #[test]
    fn dynlib_generate_lib_rs_without_css() {
        let project_root = PathBuf::from("/fake/crates/uwebr-dynlib");
        let content = "<div>Hello</div>";
        let lib_rs = generate_lib_rs(content, "App", &project_root).unwrap();
        assert!(lib_rs.contains("pub extern \"C\" fn css()"));
        assert!(lib_rs.contains("std::ptr::null()"));
    }

    #[test]
    fn dynlib_generate_lib_rs_with_css_const() {
        let project_root = PathBuf::from("/fake/crates/uwebr-dynlib");
        let content = r#"<div>Hi</div><style>.x { color: blue; }</style>"#;
        let lib_rs = generate_lib_rs(content, "Counter", &project_root).unwrap();
        assert!(lib_rs.contains("const CSS_COUNTER: &str"));
        assert!(lib_rs.contains("CSS_COUNTER.as_ptr()"));
    }

    #[test]
    fn dynlib_library_filename_various() {
        assert_eq!(abi::library_filename("App"), "uwebr_dynlib_App");
        assert_eq!(abi::library_filename("MyWidget"), "uwebr_dynlib_MyWidget");
        assert_eq!(abi::library_filename("x"), "uwebr_dynlib_x");
        assert_eq!(abi::library_filename(""), "uwebr_dynlib_");
    }

    #[test]
    fn dynlib_extract_html_removes_both_style_and_script() {
        let content = r#"<div>
<style>.a{}</style>
text1
<script>let x=1;</script>
...
<style>.b{}</style>
        ...
        assert!(!html.contains("<script>"));
        assert!(!html.contains("</script>"));
        assert!(html.contains("text1"));
        assert!(html.contains("text2"));
    }

    #[test]
    fn dynlib_extract_tag_empty_content() {
        let content = "<script></script>";
        assert_eq!(extract_tag(content, "script"), "");
    }

    #[test]
    fn dynlib_extract_css_no_closing_tag() {
        let content = "<div>Hi</div><style>.a { color: red; }";
        assert_eq!(extract_css(content), None);
    }

    #[test]
    fn dynlib_extract_html_no_tags() {
    ...
    #[test]
    fn dynlib_extract_event_handlers_none() {
        let html = r#"<div class="app">Hello</div>"#;
        let handlers = extract_event_handlers(html);
        assert!(handlers.is_empty());
    }

    #[test]
    fn dynlib_init_lib_project_creates_cargo_toml() {
        let tmp = tempfile::tempdir().unwrap();
        let project_root = PathBuf::from("/fake/crates/uwebr-dynlib");
        init_lib_project(tmp.path(), "TestComp", &project_root).unwrap();
        assert!(tmp.path().join("Cargo.toml").exists());
        assert!(tmp.path().join("src").join("lib.rs").exists());
        let toml = std::fs::read_to_string(tmp.path().join("Cargo.toml")).unwrap();
        assert!(toml.contains("uwebr_dynlib_TestComp"));
        assert!(toml.contains("cdylib"));
    }

    #[test]
    fn dynlib_transpile_script_simple_let() {
        let script = "let count = 0;";
        let output = transpile_script_simple(script);
        assert!(output.contains("static COUNT: AtomicI32 = AtomicI32::new(0);"));
        assert!(output.contains("fn get_COUNT() -> i32"));
        assert!(output.contains("fn set_COUNT(v: i32)"));
    }

    #[test]
    fn dynlib_transpile_script_simple_function() {
        let script = "function increment() { count++; }";
        let output = transpile_script_simple(script);
        assert!(output.contains("fn increment()"));
    }

    #[test]
    fn dynlib_transpile_script_simple_multiple() {
        let script = "let count = 0;\nlet step = 1;";
        let output = transpile_script_simple(script);
        assert!(output.contains("static COUNT: AtomicI32"));
        assert!(output.contains("static STEP: AtomicI32"));
    }

    #[test]
    fn dynlib_extract_tag_multiline() {
        let content = "before\n<script>\nlet x = 1;\nlet y = 2;\n</script>\nafter";
        let result = extract_tag(content, "script");
